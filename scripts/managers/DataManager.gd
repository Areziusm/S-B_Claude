...
## Charge et gère toutes les structures JSON du jeu
## ========================================================================
...
func load_all_data() -> void:
	"""Charge toutes les données JSON séquentiellement"""
	...
	# Chargement de la localisation (plus complexe)
...
func load_data_file(data_type: String) -> bool:
	...
	# Assigner aux variables appropriées
	match data_type:
		"creatures":
			creatures_db = data
...
func post_process_creatures_data() -> void:
	"""Post-traitement des données de créatures"""
...
func post_process_characters_data() -> void:
	"""Post-traitement des données de personnages"""
...
func calculate_derived_stats(creature: Dictionary) -> void:
	"""Calcule les statistiques dérivées pour une créature"""
	var base_stats = creature["base_stats"]
	
	# Exemple: calcul PV basé sur Constitution
	if base_stats.has("constitution"):
		creature["derived_hp"] = base_stats["constitution"] * 10 + 50
	
	# Calcul vitesse de mouvement
...
func validate_evolution_chain(creature_id: String, creature: Dictionary) -> void:
	"""Valide la chaîne d'évolution d'une créature"""
...
func load_localization_data(language: String) -> void:
	"""Charge les données de localisation pour une langue"""
...
func get_creature_data(creature_id: String) -> Dictionary:
	"""Récupère les données complètes d'une créature"""
	return creatures_db.get(creature_id, {})

func get_creature_evolution_data(creature_id: String, stage: int) -> Dictionary:
	"""Récupère les données d'évolution d'une créature à un stade donné"""
	var creature = get_creature_data(creature_id)
	if creature.has("evolutions"):
		var stage_key = "stage_" + str(stage)
		return creature["evolutions"].get(stage_key, {})
	return {}

func get_creatures_by_habitat(habitat: String) -> Array[String]:
	"""Retourne la liste des créatures d'un habitat donné"""
...
func get_creatures_by_rarity(rarity: String) -> Array[String]:
	"""Retourne les créatures d'une rareté donnée"""
	var result: Array[String] = []
	for creature_id in creatures_db.keys():
		var creature = creatures_db[creature_id]
		if creature.get("rarity", "common") == rarity:
			result.append(creature_id)
	return result
...
func get_characters_in_location(location: String) -> Array[String]:
	"""Retourne tous les personnages dans une localisation"""
	return cached_lookups.get("characters_by_location", {}).get(location, [])

func get_characters_by_faction(faction: String) -> Array[String]:
	"""Retourne tous les personnages d'une faction"""
	return cached_lookups.get("characters_by_faction", {}).get(faction, [])

func get_character_dialogue_tree(character_id: String) -> Dictionary:
	"""Récupère l'arbre de dialogue d'un personnage"""
	var character = get_character_data(character_id)
	var dialogue_id = character.get("dialogue_tree", "")
...
func get_quest_templates_by_type(quest_type: String) -> Array[Dictionary]:
	"""Retourne tous les modèles de quête d'un type donné"""
...
func get_quest_requirements(quest_id: String) -> Dictionary:
	"""Récupère les prérequis d'une quête"""
	var quest = get_quest_template(quest_id)
	return quest.get("requirements", {})
...
func get_item_base_price(item_id: String) -> float:
	...
	var items = economy_data.get("items", {})
	var item = items.get(item_id, {})
	return item.get("base_price", 0.0)

func get_merchant_data(merchant_id: String) -> Dictionary:
	"""Récupère les données d'un marchand"""
	var merchants = economy_data.get("merchants", {})
	return merchants.get(merchant_id, {})

func get_faction_trade_modifier(faction: String) -> float:
	"""Récupère le modificateur commercial d'une faction"""
	var faction_data = faction_relationships.get(faction, {})
	return faction_data.get("trade_modifier", 1.0)
...
func get_xp_for_level(level: int) -> int:
	...
	var xp_table = progression_tables.get("experience_table", [])
	if level > 0 and level <= xp_table.size():
		return xp_table[level - 1]
	return 0

func get_skill_unlock_level(skill_id: String) -> int:
	"""Récupère le niveau de déblocage d'une compétence"""
	var skills = progression_tables.get("skills", {})
	var skill = skills.get(skill_id, {})
	return skill.get("unlock_level", 1)

func get_reputation_threshold(reputation_level: String) -> int:
	"""Récupère le seuil de réputation pour un niveau donné"""
	var rep_table = progression_tables.get("reputation_thresholds", {})
	return rep_table.get(reputation_level, 0)
...
func load_dlc_data(dlc_id: String) -> bool:
	"""Charge toutes les données d'un DLC"""
	if not dlc_data_paths.has(dlc_id):
...
func load_dlc_data_file(dlc_id: String, data_type: String, file_path: String) -> bool:
	"""Charge un fichier de données DLC spécifique"""
	if not FileAccess.file_exists(file_path):
...
func validate_all_data() -> bool:
	"""Valide l'intégrité de toutes les données chargées"""
	var is_valid = true
	
	# Vérifier les références entre données
...
func validate_character_references() -> bool:
	"""Valide les références dans les données de personnages"""
	var is_valid = true
	
	for char_id in characters_data.keys():
		var character = characters_data[char_id]
		
		# Vérifier que le dialogue_tree existe
		var dialogue_id = character.get("dialogue_tree", "")
		if not dialogue_id.is_empty() and not dialogue_trees.has(dialogue_id):
...
func validate_quest_references() -> bool:
	"""Valide les références dans les données de quêtes"""
	var is_valid = true
	
	for quest_id in quest_templates.keys():
		var quest = quest_templates[quest_id]
		
		# Vérifier les prérequis de personnages
...
func validate_creature_references() -> bool:
	"""Valide les références dans les données de créatures"""
	var is_valid = true
...
func get_data_summary() -> Dictionary:
	"""Retourne un résumé des données chargées (debug)"""
	return {
